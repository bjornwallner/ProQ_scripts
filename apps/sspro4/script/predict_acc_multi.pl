...
#predict SA for a single sequence, SA is a combination of neural network and homology
...
#input 
	...
	# 4: sa_predictor
	# 5: script_dir
	# 6: sequence file(seq_file): one sequence in fasta format, 
	# 7: output file
	# 8: acc threshold(0:0%,..., 5:25%,...,19:95%)
...
if (@ARGV != 8)
{
	die "Usage: blast_dir big_db nr_db ss_predictor script_dir seq_file(fasta) output_file threshold\n";
}
...
if (! -f $sa_predictor)
{
	die "can't find sovlent acc predictor.\n"; 
}
...
if ($acc_threshold < 0 || $acc_threshold > 19)
{
	die "solvent accessibilty threshold index must be an integer between 0 and 19.\n";
}
...
`${script_dir}homology_sa_multi.pl $install_dir $output_prefix.tmp $output_prefix_alg  $output_prefix.accpro $acc_threshold`;
print "done\n"; 
...
`rm $output_prefix_alg`;
`rm $output_prefix.accpro`; 
